...
class StockPicking(models.Model):
    ...
    def get_pending_sale_order_line(self):
        ...
        if sale_list == []:
            raise ValidationError('All Sale Order Lines are approved.')
    ...
    def button_validate(self):
        if self.marketplace_seller_id:
            for order in self.move_ids_without_package:
                if order.sale_line_id.marketplace_state == 'pending':
                    raise ValidationError('All sale order lines linked with this delivery order should be approved first.')
